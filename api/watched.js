...
async function fetchAniListDetails(item_id) {
  try {
    const query = `
      query ($id: Int) {
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query.continue === '1') {
    try {
      const types = [
        { t: 'tmdb-movie', key: 'movies' },
        { t: 'tmdb-tv', key: 'series' },
        { t: 'anime-movie', key: 'animeMovies' },
        { t: 'anime-series', key: 'animeSeries' }
      ];
      const result = {};
      for (const { t, key } of types) {
        ...
        for (const row of rows) {
          ...
          if (t === "tmdb-movie" || t === "tmdb-tv") {
            ...
            detail = await fetchAniListDetails(row.item_id);
          }
          enriched.push({
            ...row,
            ...detail
      ...
      return res.status(500).json({ ok: false, error: 'Failed to fetch continue watching.' });
    }
  }

  // --- Batch GET watched states (unchanged) ---
  if (req.method === 'GET') {
    let items = [];
    try {
  ...
  // --- POST: Set watched state for an item (unchanged) ---
